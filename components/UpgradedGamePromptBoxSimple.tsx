...
import {
  ...
  Footprints, Skull, Plane, ChevronUp, ChevronDown,
  ChevronLeft, ChevronRight as ChevronRightIcon, X, Code
} from 'lucide-react';

// ============================================
// UPGRADED GAME PROMPT BOX - NO FRAMER MOTION
// ============================================
        ...
        <div className="flex flex-col items-center">
          {['up', 'left', 'right', 'down'].map((dir, i) => (
            <React.Fragment key={dir}>
              ...
              {(i === 0 || i === 3) && <button className="w-10 h-10 rounded-md flex items-center justify-center" style={{ backgroundColor: activeDpad === dir ? accent : '#1f2937' }} onMouseDown={() => move(dir)} onMouseUp={() => setActiveDpad(null)} onTouchStart={() => move(dir)} onTouchEnd={() => setActiveDpad(null)}>{i === 0 ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}</button>}
            </React.Fragment>
          ))}
        </div>
...
// Main Component
export default function UpgradedGamePromptBoxSimple() {
    ...
    <div className="relative">
      ...
      <div className="relative bg-gray-900/90 backdrop-blur-xl rounded-2xl border border-gray-700/50 overflow-hidden">
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-700/50 bg-gray-800/50">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-gray-400"><Cpu className="w-4 h-4" /><span className="text-sm font-medium">AI Game Engine v2.0</span></div>
          </div>
          <div className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-purple-400 animate-pulse" /><span className="text-sm text-purple-400 font-medium">Powered by Planet Q AI</span></div>
        </div>
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Describe your dream game..." className="w-full h-32 bg-gray-800/50 border border-gray-700/50 rounded-xl pl-12 pr-4 py-4 text-white placeholder-gray-500 resize-none focus:outline-none focus:border-purple-500/50" maxLength={500} />
          ...
          {/* Character */}
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-3">Character Description</label>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between pt-2">
            <div className="flex items-center gap-4 text-sm text-gray-500">
              <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-yellow-400" /><span>Instant</span></div>
              <div className="flex items-center gap-2"><Play className="w-4 h-4 text-green-400" /><span>Playable</span></div>
            </div>
            <button onClick={generate} disabled={isGenerating || !prompt.trim()} className={`flex items-center gap-3 px-8 py-4 rounded-xl font-bold text-lg transition-all ${isGenerating || !prompt.trim() ? 'bg-gray-700 cursor-not-allowed opacity-50' : 'bg-gradient-to-r from-purple-600 via-pink-600 to-cyan-600 hover:scale-105'}`}>
              {isGenerating ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />Generating...</> : <><Sparkles className="w-5 h-5 text-white" /><span className="text-white">Generate Game</span><ChevronRight className="w-5 h-5 text-white" /></>}
            </button>
          </div>
        </div>
      </div>
      ...
      {showPreview && game && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setShowPreview(false)}>
          <div className="bg-gray-900 rounded-2xl border border-gray-700 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4">
              ...
              <div className="mt-4 p-4 bg-gray-800/50 rounded-xl"><p className="text-sm text-gray-400">{game.prompt}</p></div>
              <button className="w-full mt-4 py-3 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-xl font-medium text-white flex items-center justify-center gap-2"><Code className="w-4 h-4" />Export {platform.toUpperCase()} Code</button>
            </div>
          </div>
        </div>
      )}
